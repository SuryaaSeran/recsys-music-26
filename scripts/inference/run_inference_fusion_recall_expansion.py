...
for item in tqdm(sessions, desc="Sessions"):
    ...
    for turn in conversations:
        ...
        # --- Recall expansion ---
        def add_topk(scores_arr, ids_list, k):
            ...
            for idx in top:
                ...
                if tid in cands_set or tid in seen:
                    continue
                cands.append(tid); cands_set.add(tid)

        add_topk(tt_all,   tt_ids,         args.tt_pool)
        add_topk(qm_all,   qwen_meta_ids,  args.qwen_pool)
        if cf_all is not None:
            add_topk(cf_all, cf_track_ids,  args.cf_pool)
