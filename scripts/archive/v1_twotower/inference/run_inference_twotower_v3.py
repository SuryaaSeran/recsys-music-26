"""
Hybrid BM25 + two-tower v3 inference.

Uses compact query format (user request first) to avoid 256-token truncation.
Query: latest_user_turn + goal + culture + last 2 track name/artist
...
parser.add_argument("--tid", default="twotower_v3")
parser.add_argument("--out_dir", default="exp/inference/devset")
parser.add_argument("--topk", type=int, default=20)
parser.add_argument("--bm25_pool", type=int, default=200)
parser.add_argument("--dense_weight", type=float, default=0.3)
parser.add_argument("--hist_turns", type=int, default=4)
parser.add_argument("--text_turns", type=int, default=4)
parser.add_argument("--bm25_k", type=float, default=1.0, help="BM25 RR smoothing: score=1/(rank+bm25_k). k=1 is standard RR, k=60 is RRF-style.")
parser.add_argument("--dense_pool", type=int, default=0, help="Add top-N dense candidates (not in BM25 pool) scored by raw cosine. 0=disabled.")
...
for item in tqdm(sessions, desc="Sessions"):
    ...
    for turn in conversations:
        ...
        # Compact dense query: user request first (matches training format)
        ...
        # Full BM25 query: same as run_inference_bm25_tagexpand.py for best recall
        ...
        bm25_candidates = retrieve_bm25(bm25_query, topk=retrieve_k)
        ...
        # Encode compact query with two-tower model
        query_emb = tower_model.encode(
            dense_query, normalize_embeddings=True, convert_to_numpy=True
        )

        # Score BM25 candidates
        candidate_indices = [dense_id_to_idx[t] for t in bm25_candidates if t in dense_id_to_idx]
        candidates_in_index = [t for t in bm25_candidates if t in dense_id_to_idx]
        candidates_not_in_index = [t for t in bm25_candidates if t not in dense_id_to_idx]

        if candidate_indices:
            cand_embs = dense_embeddings[candidate_indices]
            cosine_scores = cand_embs @ query_emb
        else:
            cosine_scores = np.array([])

        bm25_rr = {tid: 1.0 / (r + args.bm25_k) for r, tid in enumerate(bm25_candidates)}

        # Optional: add dense-only candidates scored by raw cosine (no BM25 rank bonus)
        dense_extra = []
        if args.dense_pool > 0:
            all_cos = dense_embeddings @ query_emb  # (47071,) cosines for all tracks
            top_dense_idx = np.argpartition(all_cos, -args.dense_pool)[-args.dense_pool:]
            top_dense_idx = top_dense_idx[np.argsort(all_cos[top_dense_idx])[::-1]]
            bm25_set = set(bm25_candidates)
            for idx in top_dense_idx:
                tid = dense_track_ids[int(idx)]
                if tid not in bm25_set and tid not in seen:
                    dense_extra.append((tid, float(all_cos[idx])))

        scored = []
        for i, tid in enumerate(candidates_in_index):
            dense_s = float(cosine_scores[i]) if i < len(cosine_scores) else 0.0
        ...
        # Dense-only candidates: score by raw cosine (no BM25 rank penalty)
        scored.extend(dense_extra)
