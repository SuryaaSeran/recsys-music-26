"""Build cross-encoder v4 training data — v8d anchor format + v8e pool.

Key fix over v3: the CE query now uses the SAME v8d role-tagged anchor format
the retrieval/LTR stack uses at inference, not the legacy [TURN-N] format.
CE v3 was unusable on v18/v20 because its query distribution didn't match.

  - Query: v8d anchor (profile + goal + reaction-tagged history + listener
    thought + current user message).
  - Hard negatives: drawn from the v8e fusion pool per turn.
  - Positive: gold track metadata text.

Output: data/crossencoder_v4/{train,valid}.jsonl
Each row: {"query": str, "docs": list[str](length 1+N), "labels": list[float]}

Usage:
    python scripts/train/build_crossencoder_v4_data.py --max_sessions 8000
"""
import os
...
# ── v8e pool config (matches v20 inference stack) ────────────────────────────
TT_MODEL   = "models/twotower_v8e/final"
TT_INDEX   = "cache/twotower_v8e"
...
parser.add_argument("--out_dir", default="data/crossencoder_v4")
parser.add_argument("--seed", type=int, default=42)
parser.add_argument("--max_turns_per_session", type=int, default=2)
parser.add_argument("--max_sessions", type=int, default=8000,
...
def get_track_short_dash(tid: str) -> str:
    row = metadata_dict.get(tid, {})
    name   = (row.get("track_name")  or [""])[0]
    artist = (row.get("artist_name") or [""])[0]
...
# ── v8d anchor builder (matches build_anchor_v8d in inference) ───────────────
from transformers import AutoTokenizer
print("Loading tokenizer for anchor budget...")
_tok = AutoTokenizer.from_pretrained("intfloat/multilingual-e5-base")
...
def build_anchor_v8d(profile, goal_text, specificity, music_hist_v8d,
                     music_labels, text_history, current_query,
    ...
    core = profile_line + "\n" + goal_line + "\n" + now_line
    budget = MAX_ANCHOR_TOK - _count_tok("query: " + core)
...
def build_rows(sessions: list, desc: str) -> list[dict]:
    ...
    for item in tqdm(sessions, desc=desc):
        ...
        text_history:  list[str] = []       # interleaved user/assistant
        thought_history: list[str] = []     # parallel to text_history
        ...
        goal_obj = item.get("conversation_goal") or {}
        goal = goal_obj.get("listener_goal", "") or ""
        specificity = goal_obj.get("specificity", "") or ""

        # gpa re-keyed: progress[t] = listener verdict on rec made at turn t
        ...
        for turn in item["conversations"]:
            ...
            if role == "music":
                ...
                if not gold_text:
                    if gold:
                        music_history.append(gold); music_turns.append(tn)
                ...
                # v8d anchor query (history excludes the current user msg = [NOW])
                mh = list(zip(music_history, music_turns))
                query = build_anchor_v8d(
                    profile, goal, specificity, mh, music_labels,
                    text_history[:-1], latest_user,
                    user_thought_history=thought_history[:-1],
                )
                ...
                thought_history.append((turn.get("thought") or "") if role == "user" else "")
