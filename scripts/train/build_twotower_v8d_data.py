"""Build two-tower v8d training data per locked spec.
...
Anchor format (role-tagged, what blind eval exposes at target turn t):
    ...
    [T2] USER: ... | REC: ... | ASST: ... | REACTION: rejected
    [NOW] USER: <Q_t>

Excluded: thought fields, target M_t. Token budget 510 (E5 max - 2).
...
parser.add_argument("--out_dir", default="data/twotower_v8d")
...
def build_anchor(profile: dict, goal_text: str, specificity: str,
    ...
    for hb in reversed(history_blocks):
        ...
        minimal = (f"[T{hb['turn']}] USER: {hb['user']} "
                   f"| REC: {hb['rec']} | REACTION: {hb['reaction']}")
...
def build_examples(sessions: list) -> list[dict]:
    ...
    for item in tqdm(sessions, desc="Building"):
        ...
        for t in item["conversations"]:
            tn = t["turn_number"]
            slot = turn_data.setdefault(tn, {"user": "", "music": "", "asst": ""})
            if t["role"] == "user":
