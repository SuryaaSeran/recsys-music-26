...
\emph{Lexical (source 1).} BM25~\cite{bm25} (via \texttt{bm25s}) indexes each track's name, artist, album,
...
an RQ-VAE assigns each track a semantic ID and a small SASRec~\cite{sasrec} predicts the next
bucket, but next-bucket prediction has a low ceiling (hit@3 = 42.1\%), so an
eight-source configuration without it reaches parity.

\emph{Collaborative (source 6).} The competition-provided BPR~\cite{bpr} matrix-factorization
...
The base encoder is \texttt{intfloat/multilingual-e5-base}~\cite{e5} (XLM-RoBERTa, 768-dim,
...
LoRA~\cite{lora} (rank 32, alpha 64) on the query/key/value projections: 1.8M trainable
...
Every session has a stated \emph{goal} (the kind of music the listener is after),
and each turn carries a \texttt{goal\_progress\_assessment} (GPA): a label recording
whether the \emph{previous} recommendation moved the listener toward that goal
(\texttt{MOVES\_TOWARD\_GOAL}) or not (\texttt{DOES\_NOT\_MOVE\_TOWARD\_GOAL}). It is
in effect per-recommendation accept/reject feedback from the listener, and the
richest preference signal in the dataset. It is also the hardest to use, for three
reasons: the raw label is off by one turn, nearly half of its ``positive'' tracks
were in fact rejected, and it is stripped out of the blind set entirely. We address
each in turn, then use the cleaned signal at inference.
...
The rescorer is a LightGBM~\cite{lightgbm} LambdaMART~\cite{lambdamart} model (nDCG@20 objective) that scores every
...
\textbf{Development and blind scores can move in opposite directions.} Adding
the progress-aware retrieval and ranking of Section~3.3 to an intermediate
two-tower model \emph{lowered} development nDCG@20 (0.1684 to 0.1615) but
\emph{raised} Blind A nDCG@20 (to 0.3701, above the prior no-GPA baseline).
The development metric alone would have rejected a change that in fact helped
on the harder blind set, where the goal, GPA, and model thoughts are all
removed. We drew two working rules from this. First, we stopped trusting the
development score in isolation: a regression on the blind set is treated as
costing more than the sum of all development-set gains, because the blind set
is the closer proxy for the real target. Second, we flag any feature whose
importance swings by more than 50\% across the five cross-validation folds as
unstable, since that instability is what lets a feature help development while
...
The system's central idea is to treat goal-progress feedback as a delayed,
sometimes-withheld preference-elicitation signal rather than a raw label: we
align it in time, remove rejected-track contamination, and reconstruct it from
dialogue when it is absent. This signal shapes recall, ranking, and the
dialogue-aware retriever's anchor alike. Combined with a union of inexpensive
retrievers arbitrated by a gradient-boosted ranker, and a single LoRA-adapted
dialogue encoder built around this signal, the system reaches composite 0.49
on the blind set, entirely on one desktop with no discrete GPU.
